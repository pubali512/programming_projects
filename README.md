...
├── nn_projects/              # Machine learning & neural network projects
│   ├── handwriting_reco/
│   ├── object_detect/
│   ├── pedestrian_detect/
...
### Machine Learning & Neural Network Projects (`nn_projects/`)
...
| **Pedestrian Detection** | A tkinter GUI application that trains a Gaussian Naive Bayes classifier to detect pedestrians in images using HOG and greyscale feature vectors (Daimler Benchmark dataset). Implements a multi-scale sliding-window prediction pipeline, overlap unification, and evaluation with confusion matrices and summary reports. |
